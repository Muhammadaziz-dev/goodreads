...
class BookReviewAPITestCase(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(username="jahongir", first_name="Jahongir")
    ...
    def test_book_review_list(self):
        ...
        book = Book.objects.create(title="Book1", description="Description1", isbn="123123")
        br = BookReview.objects.create(book=book, user=self.user, stars_given=5, comment="Nice book")
        br_two = BookReview.objects.create(book=book, user=user_two, stars_given=3, comment="Not nice book")
